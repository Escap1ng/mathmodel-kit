...
1. **候选算法生成**：针对每个小问，结合问题类型、数据规模、约束条件，从算法库中筛选 2–3 种可行算法，其中至少 1 种具有明显创新性（可引入交叉学科方法）。
...
- **算法选择与设计**：给出**算法原理**与选择该算法的理由（对比至少1种替代方案）；算法设计须有创新性（点名改进点，呼应模型建立规范第5条）且与本题结构适配，禁止一味堆砌难度模型；如采用约束分析、剪枝等搜索空间压缩策略须一并描述，给出**求解方案及算法/计算流程框图**，详尽写出算法步骤及实现方法，并列出关键参数表。算法介绍必须结合本题问题的结构与数据特点撰写，禁止通用模板式介绍文字，关键超参的选取依据须在正文阐释。
...
5. 自查流程：全文检索禁用词表（"随着…的发展""综上所述""值得注意的是""首先其次最后""具有重要意义""众所周知""较为理想""具有一定的实际意义"），命中即改写
6. 连接词频控："首先…其次…再者…最后"连接链全文至多出现一次，且仅用于单一求解流程内部的步骤引导，不用于问题之间的过渡（问间过渡一律用 (10) 的信息量句式）；每个连接词后必须跟随实质步骤内容，禁止"首先我们对数据进行了处理"式占位表述
...
**边界**：禁用"我觉得/我们认为大概/肯定最好"；任何"更好/更优"必须附数值与比较基准；个性化句子的判据是"只有这篇论文写得出"——来自本题的具体数值、具体判断与具体边界。
...
- 全文无 AI 痕迹：无"随着…的发展""综上所述""值得注意的是"等模板腔与套话；"首先…其次…最后…"连接链全文至多一处
