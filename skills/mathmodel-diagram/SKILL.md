...
模板、手写、复刻、校验、保存全在 `code/common.py` 与模板脚本内，零外部依赖。
...
三条路的绘图基元一致（同一套 `common.py`），产物可互相接着改；区别只在**流程纪律的严格程度**。
...
- **机器门禁**：每个模板写文件前逐槽跑中文字宽校验，超框报出槽位与预算并非零退出——正常的渲染应当**一次通过**，报警就值得认真看。
- **字体**：`common.py` 自动探测 SimHei → Microsoft YaHei → Noto Sans CJK → 文泉驿，全缺时告警（图中中文会是方框，必须装字体再出图）。

**不看渲染图不算画完**：代码里看不出文字溢出、箭头压字、盒子挤扁。打开 PNG 至少过两轮：① 文字溢出/压线；② 箭头方向与语义；③ 同族元素对齐同宽；④ 数值有没有抄错。完整的九区盘点与交付清单见 `docs/guides/self-check.md`。
