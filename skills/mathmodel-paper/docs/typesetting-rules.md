...
- **公式合理使用（与 [`writing-rules.md`](writing-rules.md) 的「公式使用规范」配套）**：核心公式（目标函数/关键约束/状态方程）必须 equation 独立居中编号；R²、MAE、RMSE、MAPE 等检测量公式仅首次定义一次，后续仅引用指标名+数值；禁止过度详细的分步推导公式堆砌
