...
| 表述级去 AI 检查 | 论文正文与摘要定稿后 | 词表级 `check_phrasing.py` 扫模板腔/套话/空泛（中英双语），结构级 `check_style.py` 扫句长/句式重复/段落节奏/小数位；命中即非零退出，改写后复检，风险等级「高/极高」不得交付。**表述口径以本技能为准**；中英 README、全部 `docs/` 文档与评分模块文档均须通过词表级检查（CI 从 git 派生清单，新增文档自动纳入） |
